...
class VideoEndpointsTestCase(unittest.TestCase):
    ...
    def test_get_current_videos_success(self, mock_db_cursor):
        ...
        self.assertEqual(data['videos'][0]['url'],
        ...
        self.assertEqual(data['videos'][1]['url'],
...
class GetVideosByTopicTestCase(unittest.TestCase):
    ...
    def test_get_videos_by_topic_success(self, mock_db_cursor):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            data['message'], 'videos by topic fetched successfully')
        ...
        self.assertEqual(data['videos'][2]['title'],
                         'How to repot a plant? | Beginners Guide to Repotting')
        self.assertEqual(data['videos'][2]['description'],
                         'in this video, you can learn how to repot as a beginner.')
...
class AdsTestCase(unittest.TestCase):
    ...
    def test_get_all_ads_success(self, mock_db_cursor):
        ...
        self.assertEqual(data['ads'][0]['description'],
                         'Get 15% off on all plant fertilizers!')
        self.assertEqual(data['ads'][0]['image_url'],
                         'https://example.com/images/fertilizer-ad.jpg')
        self.assertEqual(data['ads'][0]['ad_url'], 'https://example.com/sale')
        self.assertEqual(data['ads'][1]['title'], 'Plant Care Workshop')
        self.assertEqual(data['ads'][1]['description'],
                         'Join our plant care workshop and learn expert tips!')
        self.assertEqual(data['ads'][1]['image_url'],
                         'https://example.com/images/workshop-ad.jpg')
        self.assertEqual(data['ads'][1]['ad_url'],
                         'https://example.com/workshop')
        self.assertEqual(data['ads'][2]['title'], 'Plant Pot Collection')
        self.assertEqual(data['ads'][2]['description'],
                         'Explore our new collection of stylish plant pots.')
        self.assertEqual(data['ads'][2]['image_url'],
                         'https://example.com/images/pot-ad.jpg')
    ...
    def no_ads_fetched(self, mock_db_cursor):
        ...
        self.assertEqual(context.exception.message,
                         "No ads were fetched successfully.")
        self.assertEqual(context.exception.status_code, 404)
...
class VideoRatingTestCase(TestCase):
    ...
    def test_rate_video_success(self, mock_db_cursor):
        ...
        with app.test_client() as client:
            ...
            data = json.loads(response.data)
            self.assertEqual(
                data['message'], 'Rating and comment submitted successfully.')
